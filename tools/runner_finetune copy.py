...
train_transforms = transforms.Compose(
    [
        #  data_transforms.PointcloudScale(),             # 启用
         data_transforms.PointcloudRotate(),            # 启用 
        #  data_transforms.PointcloudTranslate(),         # 启用
        #  data_transforms.PointcloudJitter(),            # 启用
        #  data_transforms.PointcloudRandomInputDropout(),# 启用
         # data_transforms.RandomHorizontalFlip(),      # 可选
         data_transforms.PointcloudScaleAndTranslate(), # 保持
    ]
)
...
def get_finetune_param_groups(model, config, logger):
    """
    为微调实现分层学习率衰减 (LLRD)
    """
    optimizer_cfg = config.optimizer
    lr = optimizer_cfg.kwargs.lr
    weight_decay = optimizer_cfg.kwargs.weight_decay
    
    # 从配置中获取 LLRD 衰减率，默认为 0.75
    layer_decay_rate = optimizer_cfg.get('layer_decay_rate', 0.75) 
    
    try:
        depth = model.module.depth # 假设使用了 DataParallel
    ...
    print_log(f'[LLRD] Using Layer-wise LR Decay with rate: {layer_decay_rate}, depth: {depth}', logger=logger)

    param_groups = []
    
    # 1. [Head] 随机初始化的层：使用基础学习率
    head_params = {
        'params': [
            p for n, p in model.named_parameters() 
            if 'cls_head_finetune' in n or 'cls_token' in n or 'cls_pos' in n
        ],
        'lr': lr,
        'weight_decay': weight_decay
    }
    param_groups.append(head_params)
    print_log(f'[LLRD] Head (cls_head, cls_token, cls_pos) LR: {lr:.2e}', logger=logger)
    
    # 2. [Blocks] Transformer 块：应用指数衰减
    for i in range(depth):
        layer_lr = lr * (layer_decay_rate ** (depth - 1 - i))
        block_params = {
            'params': [
                p for n, p in model.named_parameters() 
                if f'blocks.{i}.' in n
        ...
        print_log(f'[LLRD] Block {i} LR: {layer_lr:.2e}', logger=logger)

    # 3. [Embed/Encoder] 嵌入层和 PointNet 编码器：使用最低的学习率
    lowest_lr = lr * (layer_decay_rate ** depth)
    embed_params = {
        'params': [
            p for n, p in model.named_parameters() 
            if ('encoder.' in n or 'pos_embed.' in n or 'norm.' in n) and p.requires_grad
        ],
        'lr': lowest_lr,
        'weight_decay': weight_decay
    }
    param_groups.append(embed_params)
    print_log(f'[LLRD] Embed/Encoder/Norm LR: {lowest_lr:.2e}', logger=logger)

    assigned_params = {id(p) for group in param_groups for p in group['params']}
    remaining_params = [
        p for n, p in model.named_parameters() 
        if id(p) not in assigned_params and p.requires_grad
    ]
    
    if remaining_params:
        print_log(f'[LLRD] Warning: {len(remaining_params)} params not in LLRD groups. Adding with lowest LR.', logger=logger)
        param_groups.append({'params': remaining_params, 'lr': lowest_lr, 'weight_decay': weight_decay})
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    param_groups = None
    if optimizer_cfg.get('layer_decay_rate') is not None:
        print_log('Using Layer-wise LR Decay (LLRD) for finetuning.', logger=logger)
        param_groups = get_finetune_param_groups(base_model, config, logger)
    else:
        print_log('Using single LR for all parameters.', logger=logger)
        param_groups = base_model.parameters()
    ...
    if optimizer_cfg.type == 'AdamW':
        ...
        raise NotImplementedError(f"Optimizer type {optimizer_cfg.type} not supported.")

    if scheduler_cfg.type == 'CosLR':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, 
            T_max = scheduler_cfg.kwargs.epochs,
            eta_min = 1e-6 
        )
        ...
        raise NotImplementedError(f"Scheduler type {scheduler_cfg.type} not supported.")
...
# --------------------------------------------------------------------------------------------
# 🛠️ 修复后的 validate_vote 和 test_vote 函数
# --------------------------------------------------------------------------------------------
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if npoints == 1024:
                ...
                raise NotImplementedError(f"npoints={npoints} not supported in voting")
